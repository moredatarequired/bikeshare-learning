...
class NeuralNetwork(object):
    def __init__(self, input_nodes, hidden_nodes, output_nodes, learning_rate):
        ...
        # Set self.activation_function to sigmoid.
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        hidden_error_term = np.matmul(output_error_term, self.weights_hidden_to_output.T) * hidden_outputs * (1 - hidden_outputs)
    ...
    def run(self, features):
        ...
        def f(X):
            return self.forward_pass_train(X.values)[0]
        #f = lambda X: self.forward_pass_train(X)[0]
        
        results = []
        for _, row in features.iterrows():
            #print(type(row), row.shape, row)
